...
def run_val(env,val_index):
    ...
    def update_coord(pre_coord):
        ...
        #    agent.taken_stop_action = 1 #############  MODIFICA GIGANTESCA 
        agent.v_now = v_next
    ...
    def visualization_image(graph_record,name,mask,vertices,args):
        ...
        if len(points[0]):
            if args.test:
                ...
                subfolder_name = f"validation_{exp_name}"
                full_path = os.path.join('./records/valid/vis', subfolder_name)
                os.makedirs(full_path, exist_ok=True)
                graph_map.save(os.path.join(full_path, '{}_{}.png'.format(val_index,name))) 
    ...
    for i, data in enumerate(network.dataloader_valid):
        ...
        with torch.no_grad():
            ...
            # new version (load pre-computed features)
            ###########################################
            ...
            for v_idx,init_point in enumerate(init_points):
                ...
                while 1:
                    ...
                    with torch.no_grad():
                        ...
                        #version where coord and stop are predicted jointly
                        ##########################################################  
                        pre_coord, pre_stop_action = network.decoder_coombined(cropped_feature_tensor,v_now,v_previous)
        ...
        # new version
        ######
        graph_acc, graph_recall, r_f = eval_metric(env.graph_record_np[0,0],mask,name)
        ######
        ...
        # new version
        ######
        visualization_image(env.graph_record_np,name,mask,vertices_record_list, args)
        ######

        print('Validation {}-{}/{} || graph {}/{}/{}'.format(name,i,eval_len,round(graph_acc,4),round(graph_recall,4),round(r_f_ave,4)))
        ...
        if args.test:
            ...
            exp_name = args.experiment_name
            subfolder_name = f"test_{exp_name}"

...
class FPN(nn.Module):
    def __init__(self, backbone_name = 'resnet_101', resnet_num_blocks=[2,4,23,3],n_channels=4,n_classes=1):
        ...
        # --- BACKBONE SELECTION AND CONSTRUCTION ---
        if backbone_name == 'resnet101':
            ...
            # Output channels for ResNet
            feature_channels = [256, 512, 1024, 2048] # c2, c3, c4, c5
            ...
            self.backbone = timm.create_model(
                ...
                in_chans=n_channels,
                features_only=True,
                out_indices=(1, 2, 3, 4) # Indices for EffNet (c2, c3, c4, c5)
            )
    ...
    def forward(self, x):
# --- FEATURE EXTRACTION FROM BACKBONE (MODULAR) ---
        if self.backbone_name == 'resnet101':
            # Original ResNet logic
            ...
            c4 = self.layer3(c3)
            c5 = self.layer4(c4)
        else: # EfficientNet (uses self.backbone from timm)
            # EfficientNet is a CNN, no permute needed
            # Returns c2, c3, c4, c5 directly

...
class CombinedDecoder(nn.Module):
    def __init__(self, visual_size):
        super().__init__()
        # --- 1. CORPO CONDIVISO (Shared Body) ---
        # Questa è l'architettura dei tuoi decoder, ma definita una sola volta
        self.conv = nn.Conv2d(9, 16, 1, 1)
        self.res_layer1 = BasicBlock(16, 16, stride=1)
        self.res_layer2 = BasicBlock(16, 16, stride=1)
        self.res_layer3 = BasicBlock(16, 32, stride=2)
        self.res_layer4 = BasicBlock(32, 32, stride=2)
        self.max_pool = nn.MaxPool2d(2, stride=2)
        # Questo era 'linear1' nei tuoi vecchi modelli
        self.linear_shared = nn.Linear(visual_size, visual_size // 2)

        # --- 2. TESTE SEPARATE (Separate Heads) ---
        # Due layer finali distinti, uno per compito
        self.head_coord = nn.Linear(visual_size // 2, 2) # Per le coordinate
        self.head_stop = nn.Linear(visual_size // 2, 2)  # Per l'azione di stop

    def forward(self, x, y, z):
        # --- Esegui il Corpo Condiviso UNA SOLA VOLTA ---
        x1 = self.conv(x)
        x2 = self.res_layer4(self.res_layer3(self.res_layer2(self.res_layer1(x1))))
        x3 = self.max_pool(x2)
        ...
        # Feature condivise dopo il primo layer lineare
        x5 = nn.ReLU(inplace=True)(self.linear_shared(x4))
        
        # --- Esegui le Teste Separate ---
        out_coord = self.head_coord(x5)
        out_stop = self.head_stop(x5)
        
        # Restituisci entrambi gli output in un colpo solo
        return out_coord, out_stop

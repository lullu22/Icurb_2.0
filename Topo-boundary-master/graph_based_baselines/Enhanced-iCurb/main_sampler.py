...
def run_restricted_explore(env,seq,fpn_feature_tensor, seq_lens, mask, orientation_map, name, init_points, end_points,iCurb_image_index,num,args, tolerance = 15):
    agent = env.agent
    network = env.network
    args = env.args
    train_len = network.train_len()
    def visualization_image(graph_record,epoch,iCurb_image_index,name,mask,num,vertices,args):
        ...
        # new version (gpu)
        #####
        graph_record = Image.fromarray(graph_record[0,0,:,:] * 255).convert('RGB')
        #####
        ...
        subfolder_name = f"restricted_exploration_{exp_name}"
        full_path = os.path.join('./records/train/vis', subfolder_name)
        os.makedirs(full_path, exist_ok=True)
        dst.save(os.path.join(full_path, '{}_{}_{}_{}.png'.format(epoch,iCurb_image_index,name,num)))
    ...
    for instance_id in range(instance_num):
        ...
        if len(agent.instance_vertices):
            ...
            while 1:
                ...
                env.expert_restricted_exploration(pre_coord,cropped_feature_tensor,orientation_map,True,tolerance=tolerance) # we add the tolerance 
                ...
                if agent.taken_stop_action:
                    break
    # visualization
    if args.visualization and (iCurb_image_index % 20) == 0:
        #old version(cpu)
        #####
        ...
        # new version (gpu)
        #####
        visualization_image(env.graph_record_np,env.epoch_counter,iCurb_image_index,name,mask,num,vis_generated_vertices, args)
        #####

def run_free_explore(env,seq,fpn_feature_tensor, seq_lens, mask, orientation_map, name, init_points, end_points,iCurb_image_index,num,args):
    agent = env.agent
    network = env.network
    args = env.args
    ...
    def visualization_image(graph_record,epoch,iCurb_image_index,name,mask,num,vertices, args):
        ...
        # saving with experiment name
        exp_name = args.experiment_name
        subfolder_name = f"free_exploration_{exp_name}"
        full_path = os.path.join('./records/train/vis', subfolder_name)
        os.makedirs(full_path, exist_ok=True)
        dst.save(os.path.join(full_path, '{}_{}_{}_{}.png'.format(epoch,iCurb_image_index,name,num)))   

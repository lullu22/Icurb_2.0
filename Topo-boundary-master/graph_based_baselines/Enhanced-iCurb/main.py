...
def main():
    ...
    ### Uncomment to reset directories
    #update_dir(args)

    # Print experiment name
    print(f"--- Running Experiment labeled: {args.experiment_name} ---")

    # Ensure necessary directories exist
    os.makedirs("./checkpoints", exist_ok=True)
    os.makedirs("./records", exist_ok=True)
    os.makedirs("./records/tensorboard", exist_ok=True) 
    ...
    if args.test:
        ...
        for epoch in range(args.epochs):
            ...
            # load a single one tiff image and run training on this image
                # validation mode
                if (iCurb_image_index%250)==0 and iCurb_image_index: 
                #if (iCurb_image_index == train_len -1 ) and iCurb_image_index:
                    env.network.val_mode()

...
class DatasetiCurb(Dataset):  
    ...
    '''
    def __init__(self,args, mode="valid"): # Initialize the dataset

        #
        assert mode in {"train", "valid", "test"} # Ensure the mode is valid
        #
        if args.test:
            mode = 'test'
        seq_path = args.seq_dir # Path to the sequence directory
        mask_path = args.mask_dir # Path to the mask directory
        image_path = args.image_dir # Path to the image directory

        seq_list, mask_list, image_list = load_datadir(seq_path,mask_path,image_path,mode) # Load the data directories
        self.args = args # Store the arguments
        self.seq_len = len(seq_list) # Length of the sequence list
        self.image_list = image_list # List of image paths
        self.seq_list = seq_list # List of sequence paths
        self.mask_list = mask_list # List of mask paths

...
def train_Dagger(env,data,index): # data from DAgger buffer
    r'''
    ...
    # load data
    cropped_feature_tensor, batch_v_now, batch_v_previous, crop_infos, batch_candidate_label_points, gt_stop_actions = data # [B,1,C,H,W], [B], [B], list(B), list(B), [B] where B is the batch size
    # make prediction
    cropped_feature_tensor = cropped_feature_tensor.to(env.args.device).squeeze(1) # [B,C,H,W]
...
def run_train(env,data,iCurb_image_index):
    def train():
        ...
        loss_coord_ave = 0
        loss_stop_ave = 0
        dataset = DatasetDagger(env.DAgger_buffer) # create the dataset, we only use the DAgger buffer here
        data_loader = DataLoader(dataset, batch_size=env.args.batch_size, shuffle=True,collate_fn=env.network.DAgger_collate) # create the dataloader using the dataset, the dataloader will sample batch data from the dataset
        if len(dataset): # if the dataset is not empty
            for ii, data_explore in enumerate(data_loader): # iterate through the dataloader
                loss_coord, loss_stop = train_Dagger(env,data_explore,iCurb_image_index) # train iCurb with the batch data from the dataloader
                # average the loss
                loss_coord_ave = (loss_coord_ave * ii + loss_coord) / (ii + 1) # error on the average loss for coordinate prediction
                loss_stop_ave = (loss_stop_ave * ii + loss_stop) / (ii + 1)  # error on the average loss for stop prediction
        return loss_coord_ave, loss_stop_ave

    env.training_step += 1 # count the training step
    train_len = env.network.train_len() # get the length of the training set

    ## load data ##

    ## get tiff image data ##
    seq, seq_lens, tiff, mask, name, init_points, end_points = data #
    tiff = tiff.to(env.args.device) # change device to GPU
    seq, seq_lens, mask, init_points, end_points = seq[0], seq_lens[0], mask[0], init_points[0], end_points[0] # get the data from the list
    name = name[0][-14:-5] # get the image name, we take the last part of the name string


    # extract feature of the whole image to grow the graph 
    _, fpn_feature_map = env.network.encoder(tiff) # we extract the feature map from the tiff image using the encoder
    ...
    if env.args.r_exp: # if the number of restricted exploration rounds is greater than 0
    ...
    # free exploration
    N_round = env.args.f_exp # number of free exploration rounds
    ...
    env.DAgger_buffer = [] # clear the DAgger buffer after training on the current image
    ...
    print('Epoch: {}/{} | Image: {}/{} | Loss: coord {}/stop {}'.format(
            ...
            round(loss_coord,3),round(loss_stop,3))) # print the training information 
